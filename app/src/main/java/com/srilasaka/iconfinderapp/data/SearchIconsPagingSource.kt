...
class SearchIconsPagingSource(
    ...
    private val TAG: String? = SearchIconsPagingSource::class.simpleName
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, IconsEntry> {
        ...
        return try {
            ...
            var offset = 0
            if (offsetEntry != null && offsetEntry.query.equals(query)) {
                // Increment the [OffsetEntry.offset] value if it's of same query
                offset = offsetEntry.offset + 1
            } else if (offsetEntry != null && (query != offsetEntry.query || offsetEntry.offset == 100)) {
                // Reset the [OffsetEntry] & offest, if query is different or offset value has reached it max,100.
            ...
            val offsets = icons.map {
                OffsetEntry(it.icon_id, query, offset)
            }
